describe("Form", () => {
  it("submits the form successfully with valid inputs", () => {
    ...
    cy.get('input[name="first_name"]').type("Ömer");
    cy.get('input[name="last_name"]').type("Doruk");
    cy.get('input[name="email"]').type("omer.d@example.com");
  ...
  it("displays an error message when email is already registered", () => {
    ...
    cy.get('input[name="first_name"]').type("Ömer");
    cy.get('input[name="last_name"]').type("Doruk");
    cy.get('input[name="email"]').type("omer.d@example.com"); // önceden kaydedilmiş e-posta
    cy.get('input[name="password"]').type("password123");
    cy.get('input[name="term"]').check();
    ...
    // email hatası mesajı görüntülenmesi
